...
class _SignInClientPageState extends State<SignInClientPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    PreferencesService.instance.loadRemembered().then((data){
      ...
      setState((){ _remember=remember; if(remember && email!=null) _emailController.text=email; });
    });
  }
  ...
  void _submit() async {
    ...
    try {
      ...
      if(!mounted) return;
      await PreferencesService.instance.saveRemembered(
        remember: _remember,
        email: _remember? _emailController.text.trim():null,
        role: _remember? 'CLIENT': null,
      );
      ...
      setState((){
        if(msg.toLowerCase().contains('invalid credentials') || msg.toLowerCase().contains('unauthorized')) {
          _authError='Invalid email or password.'; _emailAuthError='Invalid email or password.';
        } else { _authError='Login failed. Please try again.'; }
  ...
  double _strengthFor(String p){
    ...
    int score=0; if(p.length>=8) score++; if(RegExp(r'[a-z]').hasMatch(p)) score++; if(RegExp(r'[A-Z]').hasMatch(p)) score++;
    if(RegExp(r'[0-9]').hasMatch(p)) score++; if(RegExp(r'[^A-Za-z0-9]').hasMatch(p)) score++; return score/5.0;
  }
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      body: SignInShell(
        title: 'GigMeWork',
        subtitle: 'Find talent, scale your team',
          ...
          child: Form(
            ...
            child: Column(
              ...
              children:[
                ...
                StatefulBuilder(
                  builder: (ctx, setL) => TextFormField(
                    ...
                    decoration: signInInputDecoration(
                      ...
                      suffix: Row(mainAxisSize: MainAxisSize.min, children:[
                        ...
                        AnimatedSwitcher(duration: const Duration(milliseconds:260),
